...
import random
import re
# from functions import logger  # ✅ use the same logger as backend
...
for i in range(num_subparts):
    ...
    with col2:
        marks = st.number_input(
            f"Marks for Part ({subpart_label})",
            min_value=1, max_value=10, value=1, key=f"marks_{i}"
        )
...
# ---------------------------
chapter, concepts, topic, uploaded_file, old_concepts = None, None, None, None, None
...
# Build Subpart Block
# ---------------------------
subparts_lines = [f"  - Part ({chr(97 + i)}): [{marks_per_subpart[i]} Mark(s), {dok_levels[i]}]" for i in range(num_subparts)]
subparts_block = "\n".join(subparts_lines)
...
input_data = {
    ...
    "Number_of_questions": num_questions,
    "Number_of_subparts": num_subparts,
    "Chapter": chapter if input_mode == "Manual Input" else "",
    "Concepts": concepts if input_mode == "Manual Input" else "",
}
prompt_filled = prompt_template
for key, value in input_data.items():
    prompt_filled = prompt_filled.replace(f"{{{{{key}}}}}", str(value))
...
# ---------------------------
# Add PDF + Old Concept Notes
# ---------------------------
if input_mode == "Upload PDF":
    # logger.info("📎 Adding PDF context instructions")
    insertion_note = (
    ...
    if old_concepts and old_concepts.strip():
        ...
        old_concepts_note = (
            f"\n**IMPORTANT - Old Concepts (Previously Learned):**\n"
            f"The following concepts have been taught to students in previous grades:\n"
            f"{old_concepts}\n\n"
            f"**Instructions for using old concepts:**\n"
            f"- Use these concepts only as background understanding to ensure continuity in learning.\n"
            f"- Focus question generation on the *current topic: {topic}*.\n"
            f"- You may reference old concepts naturally if they connect with the current topic, "
            f"but do NOT create questions directly about them.\n"
            f"- Ensure that every question primarily tests understanding of new concepts.\n\n"
        )
        insertion_note += old_concepts_note

    match = re.search(r"^\s*##\s*INPUT\s*DETAILS\s*:\s*$", prompt_filled, re.IGNORECASE | re.MULTILINE)
    if match:
        start = match.end()
        prompt_filled = prompt_filled[:start] + "\n" + insertion_note + prompt_filled[start:]
    else:
...
if st.button("🚀 Generate Case Study"):
    ...
    # ✅ Build payload properly only once
    if input_mode == "Upload PDF":
        pdf_bytes = uploaded_file.read()
        base64_pdf = base64.b64encode(pdf_bytes).decode("utf-8")
        ...
        payload = {
            ...
            "input": [
                {
                    "role": "user",
                    "content": prompt_filled  # ✅ Correct format
                }
            ]
        }
    ...
    st.subheader("🧮 Token Usage")
    st.markdown(f"""
    - **Input Tokens:** {input_tokens:,} (Cached: {cached_tokens:,})
    - **Output Tokens:** {output_tokens:,} (Reasoning: {reasoning_tokens:,})
    - **Total Tokens:** {total_tokens:,}
    """)
    st.divider()
    st.subheader("💰 Cost Breakdown (USD)")
